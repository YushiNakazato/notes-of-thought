...
- 現在の利用先：学術ノート（study-tool）、M&Aテンプレートツール
- 思索日記（notes-of-thought）は独自のAstro/Fuwariテーマで、現時点ではMint UIを読み込んでいません。導入時にブラウザ検証を追加して登録します。
...
`runtimeDirectories` には、利用アプリのルートからの相対パスでローカル保存データだけを指定できます。例：M&Aツールの `data`。この領域は候補版のコピーと検証ハッシュから除外します。配信する `public/data` や `dist/data` は除外せず、教材JSONも検証・リリースの対象です。
...
- 翻訳、教材、業務データ、ページ切り替え、localStorage はアプリ側に残す。
