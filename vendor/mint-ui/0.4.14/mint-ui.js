...
/** Shared preference, persistence and OS following; applications own labels. */
export function initTheme({document: doc = globalThis.document, storageKey = 'mint-theme', defaultMode = 'dark', syncDarkClass = false, allowedModes = ['dark','light']} = {}) {
  const root=doc.documentElement, win=doc.defaultView;
  if(themeControllers.has(root)) return themeControllers.get(root);
  const modes=Object.freeze([...new Set(allowedModes)].filter(value=>['dark','light','auto'].includes(value)));
  if(!modes.length)throw Error('At least one valid theme mode is required');
  const valid=value=>modes.includes(value);
  const fallback=valid(defaultMode)?defaultMode:modes[0];
  let mode=fallback;
  try {const saved=win.localStorage.getItem(storageKey);if(valid(saved))mode=saved;} catch {}
  const media=win.matchMedia('(prefers-color-scheme: dark)');
  function apply() {
    const theme=mode==='auto'?(media.matches?'dark':'light'):mode;
    root.dataset.mintMode=mode;root.dataset.mintTheme=theme;
    if(syncDarkClass) root.classList.toggle('dark',theme==='dark');
    root.dispatchEvent(new win.CustomEvent('mint:theme',{bubbles:true,detail:{mode,theme}}));
  }
  function storage(event){if(event.key===storageKey || event.key===null){mode=valid(event.newValue)?event.newValue:fallback;apply();}}
  media.addEventListener('change',apply);win.addEventListener('storage',storage);
  const controller={modes,get mode(){return mode;},setMode(value){if(!valid(value))return false;mode=value;try{win.localStorage.setItem(storageKey,value);}catch{}apply();return true;},destroy(){media.removeEventListener('change',apply);win.removeEventListener('storage',storage);themeControllers.delete(root);}};
  themeControllers.set(root,controller);apply();return controller;
}

/** A normal Mint select bound to a theme controller. */
export function createThemeSelect({theme, label='Theme', labels={dark:'Dark',light:'Light',auto:'System'}, document:doc=globalThis.document}) {
  const control=createSelect({document:doc,label,value:theme.mode,options:(theme.modes||['dark','light']).map(value=>({value,label:labels[value]}))});
  control.element.classList.add('mint-theme-picker');
  const change=event=>theme.setMode(event.detail.value);
  const sync=()=>control.controller.setValue(theme.mode);
  control.element.addEventListener('mint:change',change);doc.documentElement.addEventListener('mint:theme',sync);
  const destroy=control.controller.destroy.bind(control.controller);
  control.controller.destroy=()=>{control.element.removeEventListener('mint:change',change);doc.documentElement.removeEventListener('mint:theme',sync);destroy();};
  return control;
}
...
export function initDisclosure(details) {
  ...
  if(details.hasAttribute('data-mint-dismissable')) {
    listen(details.ownerDocument,'pointerdown',event=>{if(!details.contains(event.target)){desiredOpen=false;settle();}},disposers);
    listen(details,'keydown',event=>{if(event.key==='Escape'&&details.open){event.preventDefault();desiredOpen=false;settle();summary.focus();}},disposers);
    listen(details,'click',event=>{if(event.target.closest('a[href]')){desiredOpen=false;settle();}},disposers);
  }
