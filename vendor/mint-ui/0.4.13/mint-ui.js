...
export function initTheme({document: doc = globalThis.document, storageKey = 'mint-theme', defaultMode = 'dark', syncDarkClass = false} = {}) {
  ...
  const fallback=valid(defaultMode)?defaultMode:'dark';
  let mode=fallback;
  ...
  const controller={get mode(){return mode;},setMode(value){if(!valid(value))return false;mode=value;try{win.localStorage.setItem(storageKey,value);}catch{}apply();return true;},destroy(){media.removeEventListener('change',apply);win.removeEventListener('storage',storage);themeControllers.delete(root);}};
...
export function createThemeSelect({theme, label='Theme', labels={dark:'Dark',light:'Light',auto:'System'}, document:doc=globalThis.document}) {
  const control=createSelect({document:doc,label,value:theme.mode,options:['dark','light','auto'].map(value=>({value,label:labels[value]}))});
